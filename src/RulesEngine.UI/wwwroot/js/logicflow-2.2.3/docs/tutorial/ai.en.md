...
For `RulesEngine.UI` (Blazor WebAssembly), the compiled LogicFlow assets are served from:

- `./src/RulesEngine.UI/wwwroot/js/logicflow-2.2.3/`
- Required runtime bundle files: `index.css` and `index.min.js`
- Local vendored docs root: `/src/RulesEngine.UI/wwwroot/js/logicflow-2.2.3/docs`

When upgrading LogicFlow, keep using the same directory pattern under `wwwroot/js/logicflow-<version>/` and update the copied prompt block below with the new active path.
...
For `RulesEngine.UI` (Blazor WebAssembly), the active local LogicFlow bundle is currently at:

- `./src/RulesEngine.UI/wwwroot/js/logicflow-2.2.3/`

Required runtime bundle files:

- `index.css`
- `index.min.js`

Local vendored docs root:

- `/src/RulesEngine.UI/wwwroot/js/logicflow-2.2.3/docs`

Versioning note:

- Keep the same pattern under `wwwroot/js/logicflow-<version>/` and update this active path when upgrading LogicFlow.
