...
对于 `RulesEngine.UI`（Blazor WebAssembly），已编译的 LogicFlow 资源从以下目录提供：

- `./src/RulesEngine.UI/wwwroot/js/logicflow-2.2.3/`
- 运行时必需文件：`index.css` 和 `index.min.js`
- 本地 vendored 文档目录：`/src/RulesEngine.UI/wwwroot/js/logicflow-2.2.3/docs`

升级 LogicFlow 版本时，继续使用 `wwwroot/js/logicflow-<version>/` 目录模式，并同步更新下方提示词中的当前生效路径。
