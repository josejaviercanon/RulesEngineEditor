...
- **THEN** it includes `RuleGuidId` (Guid), `Version` (int), `IsActive` (bool), and `Status` (string enum) with values matching persisted rule revision state
...
- **THEN** the response can represent all retained revisions for each `RuleGuidId` while preserving each rule revision's `Version`, `IsActive`, and `Status` state
...
The system SHALL provide AutoMapper mapping profiles that convert `WorkflowDto` → `RulesEngine.Models.Workflow` and `RulesEngine.Models.Workflow` → `WorkflowDto` without data loss for all supported fields, including workflow `IsEnabled`, `Comments`, and rule `Status`.
...
- **THEN** the resulting model preserves `IsEnabled`, `Comments`, `Status`, and all supported nested rule fields with equivalent values
...
- **THEN** the resulting DTO preserves `IsEnabled`, `Comments`, `Status`, and all supported nested rule fields with equivalent values
