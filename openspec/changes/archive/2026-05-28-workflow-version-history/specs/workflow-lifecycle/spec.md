...
The system SHALL support retrieving the currently active workflow revision by identifier, listing workflow revisions, and retrieving a specific retained revision by version via GET operations.

#### Scenario: Read workflow by id
- **WHEN** the client submits GET /api/workflows/{id}
- **THEN** the response returns the currently active workflow revision and its lifecycle metadata

#### Scenario: Read workflow revision history
- **WHEN** the client submits GET /api/workflows/{id}/versions
- **THEN** the response returns the retained revisions for that workflow identity in version order

#### Scenario: Read workflow by id and explicit version
- **WHEN** the client submits GET /api/workflows/{id}/versions/{version}
- **THEN** the response returns that retained revision for the workflow identity when it exists, otherwise not-found
...
The system SHALL return the active workflow revision for each workflow identity when listing workflows via GET /api/workflows, including the current version number and active-state metadata.
